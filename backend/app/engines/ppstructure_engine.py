"""
PP-StructureV3转换引擎
使用PaddleOCR的PP-StructureV3进行版面分析+OCR+表格识别+公式识别+印章识别
输出结构化Markdown，支持表格转HTML和公式转LaTeX
"""
...
class PPStructureEngine(BaseEngine):
    """PP-StructureV3版面分析引擎，输出结构化Markdown"""
    ...
    def convert_sync(self, file_path: str, file_id: str | None = None) -> str:
        ...
        with tempfile.TemporaryDirectory(prefix="transanything_ppstructure_") as temp_dir:
        ...
        if not full_result:
            logger.warning("PP-StructureV3返回内容为空")
            return ""
        ...
        if not full_markdown or not full_markdown.strip():
            logger.warning("PP-StructureV3返回空白Markdown")
            return ""
    ...
    def _get_pipeline(self) -> Any:
        ...
        with self._lock:
            ...
            try:
                ...
                raise RuntimeError("未安装paddleocr或版本不支持PPStructureV3，请升级PaddleOCR>=3.7") from exc
            ...
            try:
                ...
                raise RuntimeError(f"PP-StructureV3初始化失败: {exc}") from exc

            logger.info("PP-StructureV3初始化成功，device=%s", settings.PADDLEOCR_DEVICE)
            return self._pipeline
